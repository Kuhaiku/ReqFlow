document.addEventListener('DOMContentLoaded', () => {
    ...
    const createAndAppendCard = (req) => {
        const column = document.querySelector(`.kanban-cards[data-status="${req.status}"]`);
        if (!column) return;
        const card = document.createElement('div');
        const classMapper = (text) => text.toString().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, '-');
        card.className = `kanban-card priority-${req.prioridade} tipo-${classMapper(req.tipo)} status-${classMapper(req.status)}`;
        card.dataset.id = req.id;
        card.innerHTML = ` <div class="card-title">${req.titulo}</div> <span class="card-tag tag-${classMapper(req.tipo)}">${req.tipo}</span> <div class="card-actions"> <button class="edit-btn" title="Editar">✏️</button> <button class="delete-btn" title="Excluir">🗑️</button> </div> `;
        column.appendChild(card);
        
        // NOVO BLOCO CORRIGIDO
        card.addEventListener('click', () => {
            // Título e Descrição
            document.getElementById('view-req-title').textContent = req.titulo;
            document.getElementById('view-req-description').textContent = req.descricao;

            // Função para aplicar texto e classes nas tags do modal
            const setModalTag = (elementId, text, className) => {
                const el = document.getElementById(elementId);
                el.textContent = text;
                el.className = `card-tag ${className}`;
            };

            // Aplica as classes corretas para cada tag
            setModalTag('view-req-status', req.status, `tag-status-${classMapper(req.status)}`);
            setModalTag('view-req-priority', req.prioridade, `tag-priority-${req.prioridade}`);
            setModalTag('view-req-type', req.tipo, `tag-${classMapper(req.tipo)}`);

            viewReqModal.style.display = 'block';
        });
        
        card.querySelector('.delete-btn').addEventListener('click', async e => {
             e.stopPropagation(); if (confirm(`Excluir o requisito "${req.titulo}"?`)) { await api.request(`/api/requirements/${req.id}`, 'DELETE'); fetchAndRenderCards(); } }); card.querySelector('.edit-btn').addEventListener('click', e => { e.stopPropagation(); editingRequirementId = req.id; addReqModal.querySelector('h2').textContent = 'Editar Requisito'; addReqModal.querySelector('button[type="submit"]').innerHTML = '💾 Atualizar'; document.getElementById('req-titulo').value = req.titulo; document.getElementById('req-descricao').value = req.descricao; document.getElementById('req-tipo').value = req.tipo; document.getElementById('req-prioridade').value = req.prioridade; addReqModal.style.display = 'block'; 
        });
    };
